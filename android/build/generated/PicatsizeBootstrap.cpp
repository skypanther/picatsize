...
static Handle<Value> Picatsize_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Picatsize.getBinding requires 1 argument: binding")));
	}
...
static void Picatsize_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Picatsize_getBinding)->GetFunction());
}

static void Picatsize_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
