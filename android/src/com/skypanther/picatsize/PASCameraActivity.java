...
public class PASCameraActivity extends TiBaseActivity implements SurfaceHolder.Callback
{
	...
	public static PASCameraActivity cameraActivity = null;
	...
	public void surfaceCreated(SurfaceHolder previewHolder)
	{
		try {
			...
			onError(PicatsizeModule.UNKNOWN_ERROR, "Unable to setup preview surface: " + e.getMessage());
			cancelCallback = null;
			finish();
			return;
		}
	...
	protected void onResume()
	{
		...
		if (camera == null) {
			...
			if (whichCamera == PicatsizeModule.CAMERA_FRONT) {
	...
	{
		PASCameraActivity.cameraFlashMode = cameraFlashMode;
		if (camera != null) {
			try {
				Parameters p = camera.getParameters();
				if (cameraFlashMode == PicatsizeModule.CAMERA_FLASH_OFF) {
					p.setFlashMode(Parameters.FLASH_MODE_OFF);
				} else if (cameraFlashMode == PicatsizeModule.CAMERA_FLASH_ON) {
					p.setFlashMode(Parameters.FLASH_MODE_ON);
				} else if (cameraFlashMode == PicatsizeModule.CAMERA_FLASH_AUTO) {
	...
	private void startPreview(SurfaceHolder previewHolder)
	{
		...
		if (whichCamera == PicatsizeModule.CAMERA_FRONT) {
			cameraId = PASCameraActivity.getFrontCameraId();
		} else {
			cameraId = PASCameraActivity.getBackCameraId();
		}
		...
		if (supportedFocusModes.contains(PicatsizeModule.FOCUS_MODE_CONTINUOUS_PICTURE)) {
			param.setFocusMode(PicatsizeModule.FOCUS_MODE_CONTINUOUS_PICTURE);
	...
	private static File writeToFile(byte[] data, boolean saveToGallery) throws Throwable
	{
		try
		{
			File imageFile = null;
			if (saveToGallery) {
				imageFile = PicatsizeModule.createGalleryImageFile();
			} else {
		...
		public void onPictureTaken(byte[] data, Camera camera)
		{
			try {
				...
				if (successCallback != null) {
					...
					KrollDict response = PicatsizeModule.createDictForImage(theBlob, theBlob.getMimeType());
					successCallback.callAsync(callbackContext, response);
				}				
			} catch (Throwable t) {
				if (errorCallback != null) {
					KrollDict response = new KrollDict();
					response.putCodeAndMessage(PicatsizeModule.UNKNOWN_ERROR, t.getMessage());
	...
	private void openCamera(int cameraId)
	{
		...
		if (camera == null) {
			onError(PicatsizeModule.UNKNOWN_ERROR, "Unable to access the camera.");
			finish();
			return;
		}
	...
	{
		boolean front = whichCamera == PicatsizeModule.CAMERA_FRONT;
