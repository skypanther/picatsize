...
#define TAG "com.skypanther.picatsize"

using namespace v8;

static Persistent<Object> bindingCache;

static void Picatsize_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Picatsize.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = ::PicatsizeBindings::lookupGeneratedInit(
...
static void Picatsize_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Picatsize_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Picatsize_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::PicatsizeBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry PicatsizeBinding = {
	"com.skypanther.picatsize",
	Picatsize_init,
	Picatsize_dispose
};
...
Java_com_skypanther_picatsize_PicatsizeBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.skypanther.picatsize", &PicatsizeBinding);
	titanium::KrollBindings::addExternalLookup(&(::PicatsizeBindings::lookupGeneratedInit));
}
